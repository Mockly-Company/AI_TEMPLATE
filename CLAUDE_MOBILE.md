...
7. Path Alias: 절대 경로 사용 (@app/*, @features/*, @lib/*, @shared/*)
아키텍처
1. 디렉토리 구조: Feature-Sliced Design 영향 받은 구조 준수
2. 인증: Provider Pattern + Factory 사용 (getAuthService)
3. 상태 관리: Zustand 사용, 커스텀 훅으로 캡슐화
4. 비동기 처리: async/await + try-catch, 에러 로깅 필수
5. 테스트: 실용적이고 의미 있는 테스트 작성, Mock 타입 안전성 확보
테스트
...
환경 변수
1. NODE_ENV: 자동 로드, 수동 설정 불필요
2. 민감 정보: .env.* 파일에 저장, Git에 커밋 금지
3. 환경 변수 검증: 서비스 초기화 시 필수 변수 체크